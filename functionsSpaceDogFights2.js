	...
	function shufflePools() {// Returns an array of 4 arrays containing 4 number between 0 and 15 shuffled.
	...
	function setPoolMatches(pools) { // pools has to be an array of arrays. This function returns an array of 24 arrays. All the matches per pool.
		var poolIndex = 0;
		var resultArray = [];
	...
	function setNextsMatches() { // Query the pools variable, the demifinals variable, and the final variable. returns all the nexts matches that are known.
	...
	function resetMatch(winner, looser) { // reset match !!! In this function, decide when to find pool winner!!!
		// reset the variables.
		...
		matchCount += 1;		// matchCount +1 id here!!!
		round = 0;
	...
	function changeDogs() { // This one changes dogs and RETURNS the dogs, and does onlt that.
	...
	/*function resetChampionship() {
		// need to reset all the varable at the top.
		championships += 1;
		pools = shufflePools();
		...
		bets = [];
		currentPool = 0;
		matchCount = 0; // MatchCount starts at 0 !!!
		displayNextMatches();
		round = 0;
		count = 100;
		turn = Math.floor(Math.random()*2);
	...
	function findWinner(anyPool) { // this function choose the winner of the current pool, and pushes him into the demiFinal/demiFinalMatch variables.
	...
	function displayMatcheInfos() {
		document.getElementById("matchPlanning").innerHTML = "Current championship : " + championships + "<br>";
	...
	/*function actualizeBets() {
		if (currentBet>0 && currentBet < betList.length) {
				--currentBet;
				displayBetList();
				displayCurrentBet();
		}
